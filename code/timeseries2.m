...
%     save outlierex1.mat demp dempmt epochorg timefix eqepoch flagplotsv jq
      ...
      oflagbit=zeros(6,1);
      oflag=0;
      if ~exist('flaggpr','var')
          fprintf('\n timeseries2.m flaggpr not given, reset to 0.\n ')
          flaggpr=0;
      end %default
...
%             T6std=stdi*Pd(idkp(idsort)).^-0.5;
        ...
        for iter=1:2 %1:2 % second iteration for outlier detection using the jump fitting.
            ...
            if flaggpr==1 && iter==2
            %% GPR fitting, April 15, 2024.  https://www.mathworks.com/help/stats/gaussian-process-regression-models.html
            % openExample('stats/ComparePredictionIntervalsOfGPRModelsExample')

	    try
            gprMdl1 = fitrgp(epoch,T6);
            xt = [t1gpr:60:t2gpr]'./yr; % interval 60 days.
            [ypred1,~,yint1] = predict(gprMdl1,xt);
            ystd=abs(yint1(:,1)-ypred1);
	    %scale the std using the stdi from iteration 1; Can not apply sigma02hat to make this work.
            mstd=median(ystd(:));
	    if mstd<1e-9
	        fprintf('\n timeseries2_gpr.m: the median std from Gaussain Process Regression model is too small: %f, used the std %f from iteration 1 instead. \n',mstd,stdi);
		ystd=stdi; %Equal weight
	    else
		ystd=ystd./mstd*stdi; %scale the std using stdi. Keep the weight.
	    end
            
            %update epoch,T6,T6std 
            epoch=xt;T6=ypred1;T6std=ystd;lenf=length(T6);
            P=diag(T6std(:).^-2); 
	        %End of GPR
	    catch e
              fprintf('\n timeseries2_gpr.m lenf=%d; There was an error! The message was:\n%s',lenf,e.message);
	      return
	    end
            end  %flaggpr
...
%                fprintf(['\n No measurements before or after the event!\n'])
	    ...
	    try
            ...
            if(cdA>1e5); display(['condtion number of AM*P*AM is large: ',num2str(cdA),';jq:',num2str(jq)]); continue;end
              ...
              fprintf('\n timeseries2_gpr.m; There was an error! The message was:\n%s',e.message);
	      return
            end
...
%             est=AM\yobs; % equal weight
	    ...
	    %do not apply the posteriori reference variance for GPR, since it may be too perfect/small.
	    if ~(flaggpr==1 && iter==2)
            var=var*sigma02hat;
...
%         plot(epochorg,demp(:),'go','linewidth',2,'markersize',14) %original 
        if ~(flaggpr==1 && iter==2) %OUTliers index wrong for flaggpr
        ...
        if (flaggpr==1 && iter==2) 
            legend('GPR Models','Linear Fit');
%         legend('GPR Models','Linear Fit','GPR Uncertainty',' ',' ',' ',' ',' ',' ',' ',' ',' ','Unfiltered ArcticDEM Elevation')
        else
        legend('ArcticDEM Elevation','Linear Fit')
        end
