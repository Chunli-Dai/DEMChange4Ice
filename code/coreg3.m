...
if ~exist('flagcoregis', 'var')
    flagcoregis=0;
    fprintf('\n coreg3_bpwis2.m: flagcoregis is not given; set it to zero and only use bundle adjustment. \n')
end
...
%% Bundle adjustment
if flagcoregis==0||flagcoregis==2
...
%         if sum(size(z2out)~=size(refdem.z)) || sigma>10 || isnan(sigma) || max(abs(p(2:3))) >= 10 %control parameter
        if sum(size(z2out)~=size(refdem.z)) || ~(sigma<=maxsigma) || isnan(sigma) || ~(max(abs(p(2:3))) < maxpxpy) % in case there are NaNs in p.
            warning(['coregistration failure:',metafile]); p=zeros(3,1);perr=perrdef;
            iter=49;   
        else
            dz = z2out-refdem.z;
...
min_abs_median_dz_coregMax=min([1, min_abs_median_dz_coregMax]);
fprintf(['\n min_abs_median_dz_coregMax= ',num2str(min_abs_median_dz_coregMax),'\n']) 

%[dZ,dX,dY] = adjustOffsets(offsets,'offsetErrMax',offsetErrMax); %0.5 0.1
[dZ,dX,dY] = adjustOffsets(offsets,'offsetErrMax',offsetErrMax,'min_abs_mean_dz_coregMax',min_abs_mean_dz_coregMax,'min_sigma_dz_coregMax',min_sigma_dz_coregMax); %0.5 0.1
%  dX4Sg(j,1:3)=p;
dX4Sg1=[dZ,dX,dY];
[m,~]=size(dX4Sg1);
dX4Sg(1:m,:)=dX4Sg1;
...
data0r=[];%SAVE MEMORY.
end % if flagcoregis==0||flagcoregis==2

%% Coregister with ICESat-2 for the z direction
% flagcoregis=1; %0, only bundle adjustment and no coregistration with icesat2; 1, use icesat2 for z direction only; 2, use bundle adjustment for horizontal and icesat2 for vertical.
if flagcoregis==1||flagcoregis==2
    if flagcoregis==1 %x y are zeros.
        dX4Sg=zeros(length(idregion),3);
    end
    
    idd=[];
    for idem1=1:length(idregion) %index of DEM 1

        %read DEM 1
        i=idregion(idem1);
        metafile=[fdir{i},'/',f{i}]; 
        pn=dX4Sg(idem1,1:3);
        [npts,p,pzstd]=coregicesat(pn,metafile);
        if ~(npts>=100 && pzstd<=10) %good if we have more than 100 selected points, and the standard deviation of elevation differences is less than 10 m.
            idd=[idd;idem1];
        end
        dX4Sg(idem1,1:3)=p;
    end
    %remove strips for idregion dX4Sg
    idregion(idd)=[]; dX4Sg(idd,:)=[];
end
