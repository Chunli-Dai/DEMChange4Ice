...
%       Code written by Chunli Dai, March 26 2021.
    ...
    flagmethod=2; %1 Schaffrin's method; 2 apply isoutlier to the residuals.  %hi
...
%     est=inv(AM'*P*AM)*AM'*P*yobs;
    ...
    v2=ny-mp-1; %
    alpha=0.05; % significance level of 5%. two sigma
%     alpha=1-0.9973; % significance level. three sigma
    ...
    if 1 % apply isoutlier to residual
    fit0=fit; %linear model
    fit2=fit0+nanmedian(etilde)*ones(size(fit0));
    multi=3;
    stdi2=mad(etilde,1); %same as isoutlier
%     stdi2=mad(etilde);
    plot(ts,fit2(idsort),'g*-.','MarkerSize',12,'linewidth',4)
    plot(ts,fit2(idsort)-multi*stdi2,'r-.','linewidth',4)
    plot(ts,fit2(idsort)+multi*stdi2,'r-.','linewidth',4)           
    end
    ...
    datetick('x','mm/yy')
    box on
    legend('Data','Linear trend model','model+median of residual','green-3MAD','green+3MAD','low weight','outlier')
    ...
    if 0 %Final plot for all outliers %hi
%       Mout=isoutlier(yobs);
%       t=tobs*yr;   yobs_org=yobs;tobs_org=tobs;
    ...
    set(gcf, 'PaperPosition', [0.25 2.5 4 3]); %Default [0.25 2.5 8.0 6.0];Prefer 6 by 4.5 or 4 by 3
    hold all
    plot(t((idsort2)),yobs_org((idsort2)),'b>-','MarkerSize',12,'linewidth',4)
    if 1  %apply isoutlier to original data
    plot(t((idsort2)),fit(idsort2),'g*--','MarkerSize',12,'linewidth',4)
    plot(t((idsort2)),fit(idsort2)-multi*stdi,'r--','linewidth',4)
    plot(t((idsort2)),fit(idsort2)+multi*stdi,'r--','linewidth',4)   
    end 
    if 1 % apply isoutlier to residual
    plot(ts,fit2(idsort),'g*-.','MarkerSize',12,'linewidth',4)
    ...
    legend('Data','median','median-3mad','median+3mad','fit+ median of residual','median-3mad of residual','median+3mad of residual','Outlier')
    datetick('x','mm/yy')
    box on
    ylabel('DEM time series') 
